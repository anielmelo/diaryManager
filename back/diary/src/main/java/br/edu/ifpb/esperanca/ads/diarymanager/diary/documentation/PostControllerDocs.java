...
public interface PostControllerDocs extends IBaseController<PostRequestDTO, PostResponseDTO, Long> {
        ...
        @Operation(summary = "Listar todos os posts", description = "Retorna uma lista contendo todos os posts cadastrados no sistema.", tags = {
                        "Read" })
        @ApiResponses({
        ...
        @Operation(summary = "Listar todos os posts separados em páginas", description = "Retorna uma lista paginada de posts. Pode ser usado com parâmetros de paginação como `page`, `size`, `sort`", tags = {
                        "Read"
        })
        @ApiResponse(responseCode = "200", description = "Lista paginada de posts retornada com sucesso")
        ResponseEntity<Page<PostResponseDTO>> findAllPaged(
                        @Parameter(description = "Query de paginação dos posts", required = false) Pageable pageable);

        @Operation(summary = "Buscar post por ID", description = "Retorna os detalhes de um post específico com base no seu ID.", tags = {
                        "Read" })
        @ApiResponses({
        ...
        @Operation(summary = "Criar um novo post", description = "Cadastra um novo post no sistema com as informações fornecidas.", tags = {
                        "Create" })
        @ApiResponses({
        ...
        @Operation(summary = "Atualizar um post existente", description = "Atualiza as informações de um post previamente cadastrado.", tags = {
                        "Update" })
        @ApiResponses({
        ...
        @Operation(summary = "Excluir um post", description = "Remove permanentemente um post do sistema com base no seu ID.", tags = {
                        "Delete" })
        @ApiResponses({
